...
$description = get_field('consultation_description')
?: 'Мы поможем подобрать вариант с учетом бюджета, пожеланий и особенностей участка';
...
?>
<!-- Consultation -->
<section id="consultation" class="relative overflow-hidden bg-muted py-16 lg:py-20" <?php if (
    $bg_image
...
; ?>'); background-size: cover; background-position: center;"<?php endif; ?>>
	<?php if (!$bg_image): ?>
		...
		; ?>/assets/bg-texture.png" alt="" class="pointer-events-none absolute inset-0 h-full w-full object-cover opacity-40" aria-hidden="true">
	<?php endif; ?>
	<div class="relative mx-auto grid max-w-[1200px] items-start gap-12 px-4 lg:grid-cols-2 xl:px-0">
		<div>
			<h2 class="font-heading text-[26px] font-bold text-ink text-center lg:text-start lg:text-4xl">
				<?php echo wp_kses_post($heading); ?>
			</h2>
			<img src="<?php echo
    			esc_url(get_template_directory_uri())
			; ?>/assets/divider.svg" alt="" class="my-5 mx-auto lg:mx-0 h-[22px] w-[114px]">
			<p class="max-w-md text-base text-center lg:text-left font-light text-ink"><?php echo esc_html($description); ?></p>
			...
			<ul class="mt-10 space-y-7">
				...
				<li class="flex gap-3 flex items-center">
					<span class="flex h-18 w-18 shrink-0 items-center justify-center rounded-full text-white">
						<?php if ($icon): ?>
		...
		<div class="rounded-[16px] bg-white/80 p-[10px] shadow-lg lg:p-10 lg:ml-8">
			...
			<form class="mt-8 space-y-5" action="#" method="post">
				...
				<label class="flex items-start gap-3 text-sm text-[#6f6f6f]">
					<input type="checkbox" required checked class="checkbox-custom mt-1 h-5 min-w-5 cursor-pointer appearance-none rounded border border-[#860000] bg-white focus:ring-2 focus:ring-[#860000] focus:ring-offset-0">

					<span class="text-base">Соглашаюсь с <a href="<?php echo
    					esc_url(home_url('/privacy-policy'))
					; ?>" target="_blank" class="underline hover:text-[#860000] transition-colors">политикой обработки персональных данных</a></span>
				</label>
			...
			<ul class="mt-8 flex flex-wrap justify-center gap-8 border-t border-[#724145] pt-5 text-xs">
