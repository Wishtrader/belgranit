...
<main id="primary" class="site-main">
	...
	<!-- Hero Banner -->
	<section class="relative h-[200px] sm:h-[240px] lg:h-[334px] px-2.5 overflow-hidden mt-[72px] lg:mt-0 bg-cover bg-center bg-no-repeat" <?php if (
    	$hero_bg
		...
		<div class="max-w-[1200px] mx-auto px-4 sm:px-0">
			...
			<div class="flex flex-wrap gap-3 mb-10" id="filter-buttons">
				<a href="<?php echo esc_url(home_url('/models/')); ?>"
				   class="px-4 py-3 rounded-[6px] font-body text-sm font-light transition-all filter-btn <?php echo
    				   $all_active ? 'text-white' : 'bg-gray-100 text-[#272727] hover:bg-gray-200'
				   ; ?>"
				...
				<?php foreach ($ordered_cats as $category): ?>
					...
					<a href="<?php echo esc_url(home_url('/models/?model_cat=' . rawurlencode($category->name))); ?>"
					   class="px-4 py-3 rounded-[6px] font-body text-sm font-light transition-all filter-btn <?php echo
    					   $current_category === $category->name ? 'text-white' : 'bg-gray-100 text-[#272727] hover:bg-gray-200'
					   ; ?>"
				...
				if ($total_pages > 1): ?>
					<div class="flex justify-center items-center gap-3 mt-10">
						...
						<?php if ($current_page > 1): ?>
							...
							; ?>" class="flex items-center justify-center font-body text-[16px] text-gray-900 hover:text-[#860000] transition-colors">
						...
						<?php if ($start > 1): ?>
							...
							; ?>" class="flex items-center justify-center w-[40px] h-[40px] rounded-[6px] font-body text-[16px] text-gray-900 hover:text-[#860000] transition-colors">1</a>
							<?php if ($start > 2): ?>
						...
						<?php for ($i = $start; $i <= $end; $i++): ?>
							<?php if ($i === $current_page): ?>
								<span class="flex items-center justify-center w-[40px] h-[40px] rounded-[6px] bg-[#860000] text-white font-body text-[16px] font-semibold"><?php echo
    								esc_html($i)
								; ?></span>
							<?php else: ?>
								<a href="<?php echo
    								esc_url(add_query_arg('paged', $i, $base_url))
								; ?>" class="flex items-center justify-center w-[40px] h-[40px] rounded-[6px] font-body text-[16px] text-gray-900 hover:text-[#860000] transition-colors"><?php echo
    								esc_html($i)
								; ?></a>
							<?php endif; ?>
						<?php endfor; ?>
						...
						<?php if ($end < $total_pages): ?>
							...
							; ?>" class="flex items-center justify-center w-[42px] h-[42px] rounded-[8px] font-body text-[16px] text-gray-900 hover:text-[#860000] transition-colors"><?php echo
