...
<main id="primary" class="site-main">
	...
	<!-- Hero Banner -->
	<section class="relative px-[10px] lg:px-5 h-[300px] lg:h-[377px] overflow-hidden mt-[72px] lg:mt-0 bg-cover bg-center bg-no-repeat" <?php if (
    	$hero_bg
		...
		<div class="relative z-10 h-full max-w-[1200px] mx-auto flex flex-col justify-center">
			<nav class="font-body text-[12px] text-[#606060] mb-6 flex flex-wrap items-center gap-x-1 lg:mt-[136px]">
			...
			<?php if ($hero_subtitle): ?>
				<p class="font-manrope font-normal text-lg text-[#4c4c4c] leading-[1.2]">
			...
			<div class="rounded-[8px] bg-[#F5F4F3] p-6 lg:p-10">
				<div class="flex flex-col<?php echo $is_odd ? ' lg:flex-row' : ' lg:flex-row-reverse'; ?> gap-8 lg:gap-12">
					...
					<div class="w-full lg:w-[400px] shrink-0 flex flex-col gap-6">
						<?php if ($svc_image): ?>
							<img src="<?php echo esc_url($svc_image); ?>" alt="<?php echo
    							esc_attr($svc_title)
							; ?>" class="w-full rounded-[6px] object-cover h-[280px] lg:h-[320px]">
						<?php endif; ?>
	...
	$works_title = $page_id
    	? (get_field('improvement_works_title', $page_id) ?: 'Примеры работ по благоустройству')
    	: 'Примеры работ по благоустройству';
	...
	<section class="py-12 lg:py-20" x-data="worksSlider()">
		<div class="mx-auto max-w-[1200px] px-4 xl:px-0">
			<div class="mb-11 flex flex-col items-center gap-5">
				...
				<?php else: ?>
					...
					; ?>/assets/divider.svg" alt="" class="h-[22px] w-[114px]">
				<?php endif; ?>
			</div>

			<!-- Mobile: Swiper Slider -->
			<div class="swiper lg:hidden" id="works-swiper">
			...
			<div class="flex justify-center mt-8 lg:hidden">
				<div class="swiper works-pagination"></div>
			</div>
			<style>
				.works-pagination {
					text-align: center;
			...
			<div class="hidden lg:block overflow-hidden" x-ref="sliderContainer"
			...
			<?php if ($total_slides > 1): ?>
				<div class="hidden lg:flex justify-center gap-2 mt-8">
					<?php for ($i = 0; $i < $total_slides; $i++): ?>
						<button type="button"
							class="w-2.5 h-2.5 rounded-full transition-colors"
							:class="currentSlide === <?php echo $i; ?> ? 'bg-[#860000]' : 'bg-[#d1d5db]'"
		...
		document.addEventListener('DOMContentLoaded', function() {
			var worksSwiperEl = document.getElementById('works-swiper');
			if (worksSwiperEl) {
	...
	$content_title = $page_id
    	? (get_field('improvement_content_title', $page_id) ?: 'Благоустройство мест захоронения')
