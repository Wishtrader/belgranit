...
<main id="primary" class="site-main">
	<?php while (have_posts()):
				...
				<ol class="flex flex-wrap items-center gap-1 list-none m-0 p-0">
					...
					<?php if ($parent_term && !is_wp_error($parent_term)): ?>
						<li class="before:content-['/'] before:mx-1 before:text-gray-300">
							...
							; ?>" class="hover:text-red-700 transition-colors"><?php echo esc_html($parent_term->name); ?></a>
						</li>
					<?php endif; ?>
					<?php if ($current_term): ?>
						<li class="before:content-['/'] before:mx-1 before:text-gray-300">
							...
							; ?>" class="hover:text-red-700 transition-colors"><?php echo esc_html($current_term->name); ?></a>
						</li>
					<?php endif; ?>
		...
		if (!empty($install_items) || !empty($art_items)): ?>
			<div class="relative py-8 lg:py-20" <?php if ($svc_bg_image): ?>style="background-image: url('<?php echo
    			esc_url($svc_bg_image)
						...
						<div class="bg-white rounded-[16px] p-[10px] lg:p-6 sm:p-10 shadow-sm border border-gray-100 flex flex-col">
							...
							<div class="flex-1">
								<?php foreach ($install_items as $item):
									...
									<div class="flex justify-between items-baseline py-[10px] border-b border-gray-100">
										...
										<span class="font-semibold text-ink font-body text-sm whitespace-nowrap ml-auto mr-1"><?php echo
    										esc_html($svc_price)
										; ?> </span><span class="text-sm text-[#272727] font-light mr-3">BYN</span>
									</div>
								<?php endforeach; ?>
							</div>
							...
							<div class="mt-9 pt-4 border-t border-[#724145] flex items-center justify-between">
								...
								<div class="flex items-center gap-2">
									...
									<?php endif; ?>
									<span class="text-[#860000] font-bold text-sm font-body"><?php echo
    									esc_html($install_g_years)
									; ?> <span class="text-sm font-normal">лет</span></span>
								</div>
							</div>
						</div>
					<?php endif; ?>
							...
							<div class="flex-1">
								<?php foreach ($art_items as $item):
									...
									<div class="flex justify-between items-baseline py-3 border-b border-gray-100">
										<span class="text-[#272727] font-manrope text-sm max-w-[200px] leading-[1.2]"><?php echo
    										esc_html($svc_name)
										; ?></span>
										<span class="font-semibold text-ink font-body text-sm whitespace-nowrap ml-auto mr-1"><?php echo
    										esc_html($svc_price)
		...
		if ($granite_query->have_posts()): ?>
			<div class="max-w-[1200px] mx-auto py-4 lg:py-20 px-2.5 lg:px-0">
				...
				$all_granite = array();
				while ($granite_query->have_posts()):
    				$granite_query->the_post();
    				$all_granite[] = array(
        				'id' => get_the_ID(),
        				'title' => get_the_title(),
